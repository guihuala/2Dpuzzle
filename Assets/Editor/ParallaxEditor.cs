...
/// 视差示例编辑器扩展
/// </summary>
[CustomEditor(typeof(ParallaxExample))]
public class ParallaxExampleEditor : Editor
{
    public override void OnInspectorGUI()
    {
        DrawDefaultInspector();

        ParallaxExample parallaxExample = (ParallaxExample)target;
        ...
        if (GUILayout.Button("生成背景层"))
        {
            Undo.RecordObject(parallaxExample, "Generate Background Layers");
            parallaxExample.GenerateBackgroundLayers();
            EditorUtility.SetDirty(parallaxExample);
        }

        if (GUILayout.Button("清除背景层"))
        {
            Undo.RecordObject(parallaxExample, "Clear Background Layers");
            parallaxExample.ClearBackgroundLayers();
            EditorUtility.SetDirty(parallaxExample);
        }
